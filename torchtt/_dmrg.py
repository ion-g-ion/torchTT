...
def dmrg_matvec(A, x, y0=None, nswp=20, eps=1e-12, rmax=1024, kickrank=4, verb=False):
    if y0 == None:
        y0 = torchtt.random(
            x.N, 2, dtype=A.cores[0].dtype, device=A.cores[0].device)
    y_cores = y0.cores
    Ry = y0.R.copy()
    ...
    for i in range(nswp):
        ...
        for k in range(d-1, 0, -1):
            core = y_cores[k]
            core = tn.reshape(tn.permute(core, [1, 2, 0]), [
                              N[k]*Ry[k+1], Ry[k]])
            Q, R = QR(core)
            ...
            core_next = tn.reshape(
                y_cores[k-1], [y_cores[k-1].shape[0]*y_cores[k-1].shape[1], y_cores[k-1].shape[2]]) @ R.T
            y_cores[k-1] = (tn.reshape(core_next, [-1, N[k-1], rnew]))

            # update Phi
            # shape  rk x rAk x rxk-a x Nk
            Phi = tn.einsum('ijk,mnk->ijmn', Phis[k+1], tn.conj(x.cores[k]))
            # shape  rAk-a x Nk x rk x rxk-1
            Phi = tn.einsum('ijkl,mlnk->ijmn', tn.conj(A.cores[k]), Phi)
            # shape  rk-1 x rAk-1 x rxk-1
        ...
        for k in range(d-1):
            ...
            if not last:
                # from left
                # shape rk-1 x rAk-1 x Nk x rxk
                W1 = tn.einsum('ijk,klm->ijlm', Phis[k], tn.conj(x.cores[k]))
                # shape rk-1 x Mk x rAk x rxk
                W1 = tn.einsum('ijkl,mikn->mjln', tn.conj(A.cores[k]), W1)

                # from right
                # shape Nk+1 x rAk+1 x rxk x rk+1
                W2 = tn.einsum('ijk,mnk->njmi',
                               Phis[k+2], tn.conj(x.cores[k+1]))
                # shape rAk x Mk+1 x rxk x rk+1
            ...
            r_new = rank_chop(S.cpu().numpy(), (b.cpu()*eps /
                              (d**(0.5 if last else 1.5))).numpy())
            ...
            if i < nswp-1:
                # kick-rank
                W1, Rmat = QR(tn.cat((W1, tn.randn(
                    (W1.shape[0], kickrank), dtype=W1.dtype, device=A.cores[0].device)), axis=1))
                W2 = tn.cat(
                    (W2, tn.zeros((W2.shape[0], kickrank), dtype=W2.dtype, device=W2.device)), axis=1)
            ...
            if verb:
                print('\tcore ', k, ': delta ',
                      delta_cores[k], ' rank ', Ry[k+1], ' ->', r_new)
            Ry[k+1] = r_new
            ...
            Wc = tn.einsum('ijk,klm->ijlm',
                           tn.conj(y_cores[k]), tn.conj(y_cores[k+1]))
            ...
            # shape  rk-1 x Mk x rAk x rxk
            Phi_next = tn.einsum(
                'ijkl,jmkn->imnl', Phi_next, tn.conj(A.cores[k]))
            # shape rk x rAk x rxk
